...
public class HaloPolicy implements BatteryStateChangeCallback, NetworkSignalChangedCallback {
    ...
    private BroadcastReceiver mBatteryReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context arg0, Intent intent) {
            ...
            mCharging = intent.getIntExtra(BatteryManager.EXTRA_PLUGGED, 0);
        }
    };
    ...
    public boolean getAirplaneModeStatus() {
        if (!mCm.isNetworkSupported(ConnectivityManager.TYPE_MOBILE) && mWifiConnected)
            return false;
        return airPlaneMode;
    }
    ...
    public String getSignalStatus() {
        if (!mCm.isNetworkSupported(ConnectivityManager.TYPE_MOBILE) && mWifiNotConnected)
            return "";

        if (!mCm.isNetworkSupported(ConnectivityManager.TYPE_MOBILE))
            mSignalStrengthId = mWifiSignalIconId;
    ...
    public String getDataStatus() {
        ...
        if (mWifiConnected)
            dataContentDescription = mContext.getResources().getString(R.string.halo_wifi_on);
    ...
    public String getProvider() {
        ...
        if (mCm.isNetworkSupported(ConnectivityManager.TYPE_MOBILE))
            return removeTrailingPeriod(mLabel);

        if (mWifiLabel != null && mWifiLabel.length() > 10)
            mWifiLabel = mWifiLabel.substring(0, 9) + "...";
        if (mWifiNotConnected || mWifiLabel == null)
            mWifiLabel = mContext.getResources().getString(R.string.halo_wifi_off);
